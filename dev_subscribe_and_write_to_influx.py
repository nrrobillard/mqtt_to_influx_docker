...
# mqtt info
mqtt_username = "nick"
mqtt_password = "bigdog429"
mqtt_topic = "test"
mqtt_broker_ip = "192.168.0.144"
...
# influx info
bucket = "nickTest"
org = "ORG_NAME" # or email you used to create your Free Tier InfluxDB Cloud account
token = influx_config.INFLUX_TOKEN
# url = "http://influxdb2:8086" # for example, https://us-west-2-1.aws.cloud2.influxdata.com/
    ...
    try:
        ...
        if len(topic_parts) == 2:
            ...
            if sensor_type == 'temperature_degC':
                ...
                print(f'Temperature for {sensor_name}: {temperature_degF}°F')
                point = (
                    Point('room_temp')
                ...
                print(f'Humidity for {sensor_name}: {humidity_pct}%')
                point = (
                    Point('room_humidity')
                    .tag("location", sensor_name)
                ...
                print(f'Pressure for {sensor_name}: {pressure_hpa}hPa')
                point = (
                    Point('room_pressure')
                    .tag("location", sensor_name)
